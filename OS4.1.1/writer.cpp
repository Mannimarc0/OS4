...
int main() {
    ...
    // МАССИВ СЕМАФОРОВ ОДИН СЕМАФОР НА КАЖДУЮ СТРАНИЦУ
    HANDLE pageSem[NUM_PAGES];
    for (int i = 0; i < NUM_PAGES; i++) {
        char semName[64];
        sprintf(semName, "%s%d", SEM_PREFIX, i);
        pageSem[i] = CreateSemaphore(nullptr, 1, 1, semName);
    }
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        // ЗАХВАТ СЕМАФОРА КОНКРЕТНОЙ СТРАНИЦЫ ОЧЕРЕДЬ ЧЕСТНАЯ БЕЗ ПРИОРИТЕТА ЧИТАТЕЛЕЙ
        WaitForSingleObject(pageSem[page], INFINITE);
        ...
        // ОСВОБОЖДЕНИЕ СЕМАФОРА СТРАНИЦЫ
        ReleaseSemaphore(pageSem[page], 1, nullptr);
    ...
    CloseHandle(hMap);
    for (int i = 0; i < NUM_PAGES; i++) CloseHandle(pageSem[i]);
